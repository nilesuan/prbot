...
- **A verification pass, off by default (`PRBOT_VERIFY`).** One further call
  per chunk checks each finding against the code. A confirmed verdict can
  raise a finding's confidence to the verifier's, and no verdict lowers one,
  so verification can make a review stricter but never pass one that would
  otherwise fail. Refuted and uncertain verdicts are recorded and change
  nothing. The footer and the audit record say what was concluded, including
  each finding's confidence before its verdict, and the verifier's prompt
