...
class PrBotConfig(BaseModel, frozen=True):
    ...
    # A second call per chunk that checks each finding against the code. A
    # confirmed verdict can raise a finding's confidence and nothing lowers it
    # (review/verifier.py). Off by default, like every new feature
    # (CR-FLAG-01): turning it on is its own change. On terraform-modules MR
    # 269 it raised the finding behind a verified review's high (subnets moved
